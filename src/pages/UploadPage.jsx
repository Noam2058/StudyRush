...
export default function UploadPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const nb = createNotebook({ title, category, language: resolvedLang, questionCount: count, includeSummary, includeQuiz, sources })
    ...
    <div className="app-container app-container--with-nav">
      ...
      <main style={{ padding: 'var(--space-5)', display: 'flex', flexDirection: 'column', gap: 'var(--space-5)', maxWidth: 720, margin: '0 auto', width: '100%' }}>
        <div onClick={() => !busy && fileInput.current?.click()} style={{ border: '2px dashed var(--color-action)', background: 'var(--color-success-bg)', borderRadius: 'var(--radius-lg)', padding: 'var(--space-8) var(--space-5)', textAlign: 'center', cursor: busy ? 'not-allowed' : 'pointer', opacity: busy ? 0.6 : 1 }}>
          <input ref={fileInput} type="file" multiple accept=".pdf,.doc,.docx,.ppt,.pptx,.txt,.md" onChange={(e) => { setFiles((prev) => [...prev, ...Array.from(e.target.files || [])]); e.target.value = '' }} style={{ display: 'none' }} />
        ...
        <div>
          ...
          <InputField label={isHe ? 'שם המחברת' : 'Notebook name'} value={title} onChange={(e) => setTitle(e.target.value)} placeholder={isHe ? 'לדוגמה: ביולוגיה פרק 4' : 'e.g. Biology chapter 4'} />
          <label style={{ display: 'block' }}>
            <span style={{ display: 'block', fontSize: 'var(--font-size-small)', fontWeight: 600, marginBottom: 'var(--space-2)', color: 'var(--color-text-secondary)' }}>{isHe ? 'קטגוריה' : 'Category'}</span>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)', marginBottom: 'var(--space-4)' }}>
            {[['includeSummary', includeSummary, setIncludeSummary, isHe ? 'צור סיכום AI' : 'Generate AI summary'], ['includeQuiz', includeQuiz, setIncludeQuiz, isHe ? 'צור שאלות AI' : 'Generate AI quiz']].map(([, val, setter, lbl]) => (
